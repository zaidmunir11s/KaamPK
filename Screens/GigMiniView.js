...
export default class GigMiniView extends React.Component {
  ...
  horizontalScroll() {
    ...
    return (
      ...
      >
        <View style={{ width: RFValue(167), height: RFValue(150) }}>
          <Image source={{ uri: this.props.ImageUri }} style={{
            resizeMode: "stretch", width: '100%',
            height: '100%'
          }} />
          <Text style={[styles.Text, { fontSize: RFValue(16), fontWeight: "bold" }]}>{this.props.GigTitle}  </Text>
        </View>
        <View style={{ flexDirection: "column", flex: 2, justifyContent: 'flex-end' }}>
          ...
          <View style={{ width: RFValue(160), flexDirection: "row", margin: RFValue(5) }} >
            ...
            <Text style={{
              alignSelf: "center", fontWeight: "bold", fontSize: this.fontP, marginLeft: RFValue(35), color: 'white', backgroundColor: '#272b2b',
          ...
          <View style={{ backgroundColor: 'white', width: RFValue(120), flexDirection: "row", borderRadius: RFValue(5), margin: RFValue(5) }} >
            <Text style={{ color: '#009387', alignSelf: "center", fontSize: this.fontS, fontWeight: "bold", margin: RFValue(5) }}>{this.props.item.username}</Text>
  ...
  render() {
    return (
      this.props.scroll ? this.horizontalScroll() : this.verticalScroll()
