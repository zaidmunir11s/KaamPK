...
export default class NotificationSellerDashboard extends React.Component {
  ...
  getData = () => {
    const users = []
    const subscriber = NotificationController.CheckforNotification(this.props.route.params.Seller, (data) => {
          ...
          {this.checkFlag() ? <Text style={{ alignSelf: "center", fontSize: 20 }}>No Notification</Text> :
            <FlatList
              ...
              renderItem={({ item }) => (
                ...
                <NotificationSeller
                  thiss={this.getData.bind(this)} OrderInfo={item} Buyer={item.From} Gig={item.GigDetails}
                  navigation={this.props.navigation}></NotificationSeller>
